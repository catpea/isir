...
function parse(input, buffer) {
    ...
    if (process.arch === 'x64') {
        event = {
            timeS: buffer.readUIntLE(0, 4),
            timeMS: buffer.readUIntLE(8, 4),
            type: buffer.readUInt16LE(16, 2),
            code: buffer.readUInt16LE(18, 2 ),
            value: buffer.readInt32LE(20, 4),
        };
    } else { // arm or ia32
        event = {
            timeS: buffer.readUInt32LE(0, 4),
            timeMS: buffer.readUInt32LE(4, 4),
            type: buffer.readUInt16LE(8, 2),
            code: buffer.readUInt16LE(10, 2 ),
            value: buffer.readInt32LE(12, 4),
        };
    }
